var socketIO = function (io, socket, onlineUsers) {
  ...
  socket.on('message', (data) => {
    ...
    console.log(`message from room ${data.roomId} - ${data.displayName}: ${data.msg} : ${data.seen}`);
    io.to(data.roomId).emit('receivedMsg', { avatar:data.avatar, displayName:data.displayName, msg:data.msg , seen: data.seen});
    } catch (err){
      console.log(err);
    }
  });
